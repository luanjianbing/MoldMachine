...
Public Class Form1
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        InitialDiDgViewModbusGeneralRow(m_DiValueStartAddr, m_iDiTotal, listViewDiChannelInfo)

        InitialAoDgViewModbusGeneralRow(m_iAoValueStartAddr, m_iAoChTotal, listViewAoChannelInfo)
        ...
        RefreshAoModbusValue(m_iAoValueStartAddr, m_iAoChTotal, listViewAoChannelInfo)
        cbxAoChannel.SelectedIndex = 0
    End Sub
    Private Sub InitialDiDgViewModbusGeneralRow(ByVal iDiChStart As Integer, ByVal iDiChTotal As Integer, ByRef targetListView As ListView)
        Dim iPos, iIdx As Integer
        targetListView.Items.Clear()
        Dim listItemsDevice As ListViewItem

        iPos = iDiChStart
        For iIdx = 0 To (iDiChTotal - 1)
            'Text type : Location
            listItemsDevice = New ListViewItem(iPos.ToString("D5"))
            'Text type : Type
            listItemsDevice.SubItems.Add("DI")
            'Text type : Ch No.
            listItemsDevice.SubItems.Add(iIdx.ToString())
            'Text type : Bool
            listItemsDevice.SubItems.Add("BOOL")
            'Text type : Mode Value
            listItemsDevice.SubItems.Add("")

            targetListView.Items.Add(listItemsDevice)
    ...
    Private Sub InitialAoDgViewModbusGeneralRow(ByVal iAiChStart As Integer, ByVal iAiChTotal As Integer, ByRef targetListView As ListView)
        Dim iPos, iIdx As Integer
        targetListView.Items.Clear()
        Dim listItemsDevice As ListViewItem
        ...
        For iIdx = 0 To (iAiChTotal - 1)
            ...
            listItemsDevice = New ListViewItem(iPos.ToString("D5"))
            'Text type : Type
            listItemsDevice.SubItems.Add("AO")
            'Text type : Ch No.
            listItemsDevice.SubItems.Add(iIdx.ToString())
            'Text type : Value[Dec]
            listItemsDevice.SubItems.Add("******")
    ...
    Private Function RefreshAoModbusValue(ByVal i_AoValueStart As Integer, ByVal i_AoChTotal As Integer, ByRef listViewModbus As ListView) As Boolean
        Dim iIdx As Integer
        ...
        If (adamModbus.Modbus().ReadInputRegs(i_AoValueStart, i_AoChTotal, iValueData)) Then
            For iIdx = 0 To (i_AoChTotal - 1)
                ...
                listViewModbus.Items(iIdx).SubItems(3).Text = m_usAoValue(iIdx).ToString()
                listViewModbus.Items(iIdx).SubItems(4).Text = m_usAoValue(iIdx).ToString("X04")
                listViewModbus.Items(iIdx).SubItems(5).Text = AnalogOutput.GetScaledValue(m_Adam6000Type, m_usRange(iIdx), m_usAoValue(iIdx)).ToString(szFormat)
                listViewModbus.Items(iIdx).SubItems(6).Text = szRange
            Next iIdx
            Return True
        End If
        Return False
    ...
    Private Function RefreshDiModbusValue(ByVal i_DiValueStart As Integer, ByVal i_DiChTotal As Integer, ByRef listViewModbus As ListView) As Boolean
        Dim iIdx As Integer
        ...
        If (adamModbus.Modbus().ReadCoilStatus(i_DiValueStart, i_DiChTotal, iValueData)) Then
            ...
            For iIdx = 0 To (i_DiChTotal - 1)
                listViewModbus.Items(iIdx).SubItems(4).Text = iValueData(iIdx).ToString()
            Next iIdx
            Return True
        End If
        Return False
    ...
    Private Sub Timer1_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Timer1.Tick
        ...
        If (tabControlMain.SelectedIndex = 0) Then
            If (RefreshDiModbusValue(m_DiValueStartAddr, m_iDiTotal, listViewDiChannelInfo)) Then
                txtReadCount.Text = "Read Coil Status " + m_iCount.ToString() + " times..."
            Else
                txtReadCount.Text = "Read Coil Status failed !"
            End If
        Else
            If (RefreshAoModbusValue(m_iAoValueStartAddr, m_iAoChTotal, listViewAoChannelInfo)) Then
        ...
        If (cbxAoChannel.SelectedIndex <> m_iAoChTotal) Then
            If (Timer1.Enabled) Then
                ...
                If (cbxAoChannel.SelectedIndex >= 0) Then
                    listViewAoChannelInfo.Items(cbxAoChannel.SelectedIndex).Selected = True
                End If
                Timer1.Enabled = True
            Else
                RefreshAoChannelRange(cbxAoChannel.SelectedIndex, True)
            ...
            panelAoValueSetting.Visible = True
            ...
            If (iRetryCount >= iRetryLimit) Then
                MessageBox.Show("AnalogOutput.GetRangeHighLow failed! Please retry again.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Exclamation, MessageBoxDefaultButton.Button1)
            End If
        Else
            panelAoValueSetting.Visible = False
        End If
    End Sub
    ...
    Private Sub ApplyAoOutputRange(ByVal idxChannel As Integer, ByVal idxOutputRange As Integer, ByVal bShowDone As Boolean)
        ...
        If (idxChannel <> m_iAoChTotal) Then
            ...
            Else
                MessageBox.Show("Change output range failed!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, MessageBoxDefaultButton.Button1)
            End If
        Else
            ...
            If (bRet) Then
                ...
                Next iChannel
            Else
                MessageBox.Show("Change range configuration failed!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, MessageBoxDefaultButton.Button1)
            End If
        End If
    End Sub

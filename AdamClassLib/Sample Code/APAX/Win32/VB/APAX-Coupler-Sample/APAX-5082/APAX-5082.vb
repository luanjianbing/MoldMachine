...
Public Class Form_APAX_5082
    ...
    Public Sub InitialRemotePanelComponents()
        ...
        For iLoop = 0 To Me.m_iDOChannelNum - 1
            ...
            lvItem.SubItems.Add(iLoop.ToString)  'Ch
            Dim szTmp As String = (Convert.ToString(((m_usStart - 40001) * 2 + iLoop + 1 + m_iDIChannelNum)) + ("[" + (Convert.ToString((iLoop)) + "]")))
            lvItem.SubItems.Add(szTmp)  'modbus address
            lvItem.SubItems.Add("*****")  'Value
            lvItem.SubItems.Add("BOOL") 'Mode
            lvItem.SubItems.Add("*****") 'Safety
    ...
    Private Function RefreshPwmSetting() As Boolean ' Update PWM Channel Information "Frequency" and "Duty Cycle" columns
        ...
        Do While (i < m_iPWMChannelNum)
            If m_adamModbusSocket.Counter.GetChannelConfig(CType(Me.m_iSlot_ID, Integer), CType(i, System.UInt16), uiFerquency(i), fDutyCycle(i)) Then
                listViewChInfo_PWM.Items(i).SubItems(4).Text = uiFerquency(i).ToString
                'Update Frequency column
                listViewChInfo_PWM.Items(i).SubItems(5).Text = fDutyCycle(i).ToString("0.0")
    ...
    Private Sub RefreshPwmSettingFormInfo() ' Update PWM channel Setting -> Frequency and DutyCycle
        ...
        If m_adamModbusSocket.Counter.GetChannelConfig(CType(Me.m_iSlot_ID, Integer), CType(iSelectChannelIndex, System.UInt16), uiFerquency, fDutyCycle) Then
    ...
    Private Sub RefreshPwmChannelMask() ' refresh PWM Channel status
        ...
        If m_adamModbusSocket.Configuration.GetModuleCurrentChMsk(Me.m_iSlot_ID, m_aConf) Then 'refresh PWM Channel status
    ...
    Private Function RefreshData() As Boolean
        ...
        If (strSelPageName = "DI") Then
            ...
            For i = 0 To bInVal.Length - 1
                If listViewChInfo_DI.Items(i).SubItems(3).Text <> bInVal(i).ToString Then
                    listViewChInfo_DI.Items(i).SubItems(3).Text = bInVal(i).ToString
        ...
        ElseIf (strSelPageName = "DO") Then
            ...
            If Not m_adamModbusSocket.DigitalOutput.GetValues(Me.m_iSlot_ID, Me.m_iDOChannelNum + m_iDIChannelNum, bOutVal) Then
                Me.StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" _
            ...
            For i = 0 To bOutVal.Length - m_iDIChannelNum - 1
                If listViewChInfo_DO.Items(i).SubItems(3).Text <> bOutVal(i + m_iDIChannelNum).ToString Then
                    listViewChInfo_DO.Items(i).SubItems(3).Text = bOutVal(i + m_iDIChannelNum).ToString
                    'moduify "Value" column
                End If
            Next
    ...
    Private Sub settingPWM_MaskSetting(ByVal bEnable As Boolean, ByVal i_bApplyAll As Boolean) ' Set PWM channel setting -> Output status
        ...
        If ((listViewChInfo_PWM.SelectedIndices.Count = 0) AndAlso Not i_bApplyAll) Then
            MessageBox.Show("Please select the target channel in the listview!", "Information", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
            bRet = False
        End If
        If bRet Then
            ...
            Else
                ...
                If bEnable Then
                    strOut = "Set Channel Start failed! Err:" + m_adamModbusSocket.Counter().ApiLastError.ToString()
                Else
                    strOut = "Set Channel Stop failed! Err:" + m_adamModbusSocket.Counter().ApiLastError.ToString()
                End If
            ...
            If bRet Then
                ...
                Do While (i < listViewChInfo_PWM.Items.Count)
                    If chbxApplyAll.Checked Then
                        If m_adamModbusSocket.Counter.SetChannelConfig(Me.m_iSlot_ID, CType(i, System.UInt16), uiFreq, fDuty) Then
                            RefreshPwmSetting()
                        Else
    ...
    Private Sub Form_APAX_5082_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        FreeResource()
    End Sub
End Class

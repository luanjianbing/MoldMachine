...
Public Class Form_APAX_5080
    ...
    Dim m_usRanges_supCNT() As System.UInt16
    ...
    Public Function DeviceFind() As Boolean
        ...
        If iDeviceNum = 1 Then
            DeviceFind = True
        ElseIf iDeviceNum > 1 Then 'the same Devices exist two more or more
    ...
    Public Function RefreshConfiguration() As Boolean
        ...
        If m_adamModbusSocket.Configuration.GetModuleConfig(m_iSlot_ID, m_aConf) Then
            ...
            m_adamModbusSocket.Configuration.GetModuleCurrentRange(Convert.ToInt32(m_iSlot_ID), m_aConf)
            m_usRanges = m_aConf.wChRange
            m_adamModbusSocket.Configuration.GetModuleCurrentChMsk(Convert.ToInt32(m_iSlot_ID), m_aConf)
    ...
    Public Sub InitialRemotePanelComponents()
        ...
        For iLoop = 0 To Me.m_iDIChannelNum - 1
            ...
            Dim szTmp As String = (Convert.ToString(((m_usStart - 40001) * 2 + iLoop + 1)) + ("[" + (Convert.ToString((iLoop)) + "]")))
        ...
        For iLoop = 0 To m_iDOChannelNum - 1
            ...
            lvItem.SubItems.Add(iLoop.ToString)  'Ch
            Dim szTmp As String = (Convert.ToString(((m_usStart - 40001) * 2 + iLoop + 1 + m_iDIChannelNum)) _
             + ("[" + (Convert.ToString((iLoop)) + "]")))
            lvItem.SubItems.Add(szTmp)  'modbus address
        ...
        For iLoop = 0 To strRange.Length - 1
            strRange(iLoop) = Counter.GetModeName(m_usRanges_supCNT(iLoop))
        Next
        ...
        For iLoop = 0 To m_iCNTChannelNum - 1
            ...
            lvItem.SubItems.Add(iLoop.ToString) 'Ch
            Dim szMbAddr As String = (Convert.ToString((m_usStart + (iLoop * 2))) + ("[H] " + (Convert.ToString((m_usStart + ((iLoop * 2) + 1))) + "[L]")))
    ...
    Public Function GetChannelInfo() As Boolean
        ...
        For iLoop = 0 To m_aConf.HwIoType.Length - 1
            ...
            If (m_aConf.HwIoType(iLoop) = _HardwareIOType.CNT) Then
                ...
                m_CNTidx = iLoop
                If (iLoop = 0) Then
                    m_adamModbusSocket.Configuration().GetModuleTotalRange(CType(m_iSlot_ID, Integer), m_aConf, 0)
                    m_usRanges_supCNT = m_aConf.wHwIoType_0_Range
                ElseIf (iLoop = 1) Then
                    m_adamModbusSocket.Configuration().GetModuleTotalRange(CType(m_iSlot_ID, Integer), m_aConf, 0)
                    m_usRanges_supCNT = m_aConf.wHwIoType_1_Range
                ElseIf (iLoop = 2) Then
                    m_adamModbusSocket.Configuration().GetModuleTotalRange(CType(m_iSlot_ID, Integer), m_aConf, 0)
                    m_usRanges_supCNT = m_aConf.wHwIoType_2_Range
                ElseIf (iLoop = 3) Then
                    m_adamModbusSocket.Configuration().GetModuleTotalRange(CType(m_iSlot_ID, Integer), m_aConf, 0)
                    m_usRanges_supCNT = m_aConf.wHwIoType_3_Range
                Else
                    m_adamModbusSocket.Configuration().GetModuleTotalRange(CType(m_iSlot_ID, Integer), m_aConf, 0)
                    m_usRanges_supCNT = m_aConf.wHwIoType_4_Range
                End If
    ...
    Private Sub RefreshDoAlarmSetting()
        ...
        Do While (i < listViewChInfo_DO.Items.Count)
            ...
            If m_adamModbusSocket.Alarm.GetLocalAlarmConfiguration(m_iSlot_ID, i, bEnable, bAutoRL, byAlarmtype, iMappingCh, uiLimitVal, byDOBehavior, uiPulseWidth) Then
                ...
                If bEnable Then
                    listViewChInfo_DO.Items(i).SubItems(1).Text = "Alarm"
                    'moduify "Type" column
                    ...
                    End If
                    listViewChInfo_DO.Items(i).SubItems(8).Text = strMappingCh
                    'moduify "Map Ch" column
                    listViewChInfo_DO.Items(i).SubItems(6).Text = uiLimitVal.ToString
                    ...
                    End If
                Else
                    listViewChInfo_DO.Items(i).SubItems(1).Text = "DO"
                    'moduify "Type" column
                    ...
                    'moduify "Map Ch" column
                    listViewChInfo_DO.Items(i).SubItems(6).Text = "Disable"
                    'moduify "Alarm Limit" column
                    listViewChInfo_DO.Items(i).SubItems(9).Text = "Disable"
                    'moduify "DO Behavior" column
                End If
            Else
                listViewChInfo_DO.Items(i).SubItems(1).Text = "*****"
                'moduify "Type" column
                listViewChInfo_DO.Items(i).SubItems(5).Text = "*****"
                'moduify "Alarm Type" column
                listViewChInfo_DO.Items(i).SubItems(8).Text = "*****"
                'moduify "Map Ch" column
                listViewChInfo_DO.Items(i).SubItems(6).Text = "*****"
                'moduify "Alarm Limit" column
                listViewChInfo_DO.Items(i).SubItems(9).Text = "*****"
                'moduify "DO Behavior" column
                Dim strErr As String
                strErr = ("GetLocalAlarmConfiguration(Error:" _
                            + (m_adamModbusSocket.Counter.ApiLastError.ToString + ") Failed! "))
    ...
    Private Sub RefreshCntCountMode()
        ...
        Do While (i < listViewChInfo_CNT.Items.Count)
            If m_adamModbusSocket.Counter.GetCntTypeConfiguration(Me.m_iSlot_ID, i, bOnceRepeat, bReloadIniti) Then
                ...
                End If
                listViewChInfo_CNT.Items(i).SubItems(8).Text = strCntMode
                'Update "Count Type" column
            Else
                listViewChInfo_CNT.Items(i).SubItems(8).Text = "*****"
    ...
    Private Sub RefreshCntGateSetting()
        ...
        Dim i As Integer = 0
        Do While (i < listViewChInfo_CNT.Items.Count)
            If m_adamModbusSocket.Counter.GetLocalGateConfiguration(m_iSlot_ID, i, bEnable, byRetriggerMode, byGatetype, iMappingCh) Then
                ...
                End If
                listViewChInfo_CNT.Items(i).SubItems(11).Text = strGateSetting
                'Update "Gate Trigger" column
                listViewChInfo_CNT.Items(i).SubItems(10).Text = strGateActiveType
                'Update "Gate Active" column
                listViewChInfo_CNT.Items(i).SubItems(9).Text = strMappingCh
                'Update "Map Ch" column
            Else
    ...
    Private Function RefreshCntStartupValues() As Boolean
        ...
        Try
            ...
            If m_adamModbusSocket.Counter.GetStartupValues(m_iSlot_ID, iChannelTotal, m_uiStartupCNT) Then
                Dim i As Integer = 0
                Do While (i < iChannelTotal)
                    If (Not ((m_usRanges(i) = CType(ApaxUnknown_InputRange.Up, System.UInt16)) _
                        ...
                        'TODO: Warning!!! continue If
                    End If
                    listViewChInfo_CNT.Items(i).SubItems(5).Text = m_uiStartupCNT(i).ToString
                    'Update "Startup" column
                    i = (i + 1)
                Loop
                Return True
    ...
    Private Sub RefreshCntSetting()
        ...
        If m_adamModbusSocket.Counter.GetDigitalFilter(m_iSlot_ID, iFilter) Then
        ...
        Try
            If m_adamModbusSocket.Configuration.GetModuleConfig(m_iSlot_ID, m_aConf) Then
                ...
                For i = 0 To strRange.Length - 1
                    ...
                    End If
                    listViewChInfo_CNT.Items(i).SubItems(4).Text = strRange(i).ToString

                    'Update "Mode" column
                    If m_bChMask(i) Then
                        listViewChInfo_CNT.Items(i).SubItems(6).Text = "Start"
                    Else
                        ...
                        If (Not ((m_usRanges(i) = CType(ApaxUnknown_InputRange.Up, System.UInt16)) _
                                    ...
                                    OrElse (m_usRanges(i) = CType(ApaxUnknown_InputRange.WaveWidth, System.UInt16))))) _
                                    AndAlso ((i Mod 2) = 1)) Then
                            listViewChInfo_CNT.Items(i).SubItems(6).Text = "*****"
                            'Update "Counting" column
                        Else
                            listViewChInfo_CNT.Items(i).SubItems(6).Text = "Stop"
                            'Update "Counting" column
                        End If
                    End If
                Next
    ...
    Private Function RefreshData() As Boolean ' Refresh Channel Information "Value" column
        ...
        If (strSelPageName = "DI") Then
            ...
            If Not m_adamModbusSocket.DigitalInput.GetValues(Me.m_iSlot_ID, m_iDIChannelNum, bVal) Then
                StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" + (m_adamModbusSocket.DigitalInput.ApiLastError.ToString + " ")))
                Return False
            End If
        ...
        ElseIf (strSelPageName = "DO") Then
            ...
            If Not m_adamModbusSocket.DigitalOutput.GetValues(m_iSlot_ID, (m_iDIChannelNum + m_iDOChannelNum), bVal) Then
                StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" + (m_adamModbusSocket.DigitalOutput.ApiLastError.ToString + " ")))
                Return False
            End If
            ...
            If Not m_adamModbusSocket.Alarm.GetAlarmFlags(m_iSlot_ID, uiVal) Then 'Refresh AlarmFlag
                StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" + (m_adamModbusSocket.Alarm.ApiLastError.ToString + " ")))
                Return False
            End If
        ...
        ElseIf (strSelPageName = "CNT") Then
            ...
            If Not m_adamModbusSocket.Counter.GetChannelStatus(Me.m_iSlot_ID, m_iCNTChannelNum, aStatus) Then
                StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" + (m_adamModbusSocket.Counter.ApiLastError.ToString + " ")))
                Return False
            End If
            If Not m_adamModbusSocket.Counter.GetValues(m_iSlot_ID, uiVal) Then
                StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" _
                            + (m_adamModbusSocket.Counter.ApiLastError.ToString + " ")))
                Return False
            End If
            ...
            For i = 0 To Me.m_iCNTChannelNum - 1
                ...
                If Not m_adamModbusSocket.Counter.GetGateTriggerStatus(m_iSlot_ID, i, o_bStatus) Then
                    StatusBar_IO.Text = (StatusBar_IO.Text + ("ApiErr:" _
                                + (m_adamModbusSocket.Counter.ApiLastError.ToString + " ")))
                End If
                If o_bStatus Then
                    If (listViewChInfo_CNT.Items(i).SubItems(11).Text.Substring((listViewChInfo_CNT.Items(i).SubItems(11).Text.Length - 3), 3) <> "(*)") Then
                        listViewChInfo_CNT.Items(i).SubItems(11).Text = (listViewChInfo_CNT.Items(i).SubItems(11).Text + "(*)")
                    End If
                ElseIf (listViewChInfo_CNT.Items(i).SubItems(11).Text.Substring((listViewChInfo_CNT.Items(i).SubItems(11).Text.Length - 3), 3) = "(*)") Then
                    listViewChInfo_CNT.Items(i).SubItems(11).Text = listViewChInfo_CNT.Items(i).SubItems(11).Text.Substring(0, (listViewChInfo_CNT.Items(i).SubItems(10).Text.Length - 3))
                End If
                If (Not ((m_usRanges(i) = CType(ApaxUnknown_InputRange.Up, System.UInt16)) _
                            OrElse ((m_usRanges(i) = CType(ApaxUnknown_InputRange.HighFrequency, System.UInt16)) _
                            OrElse ((m_usRanges(i) = CType(ApaxUnknown_InputRange.LowFrequency, System.UInt16)) _
    ...
    Private Sub tcRemote_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tcRemote.SelectedIndexChanged
        ...
        ElseIf (strSelPageName = "DO") Then
            ...
            If (listViewChInfo_DO.SelectedIndices.Count = 0) Then
                listViewChInfo_DO.Items(1).Selected = True
            End If
        ElseIf (strSelPageName = "CNT") Then
            RefreshRanges()
            ...
            If (listViewChInfo_CNT.SelectedIndices.Count = 0) Then
                listViewChInfo_CNT.Items(1).Selected = True
            End If
        End If
    ...
    Private Sub btnAlarmClear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAlarmClear.Click
        ...
        Do While (i < listViewChInfo_DO.SelectedIndices.Count)
            ...
            i = (i + 1)
        Loop
        If m_adamModbusSocket.Alarm.ClearAlarmFlags(Me.m_iSlot_ID, uiMask) Then
            RefreshData()
        Else
            MessageBox.Show(("Clear alarm flags failed! ApiErr:" _
                            + (m_adamModbusSocket.Alarm.ApiLastError.ToString + "")), "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, MessageBoxDefaultButton.Button1)
        End If
        Timer1.Enabled = True
    End Sub
    ...
    Private Sub btnAlarmClearall_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAlarmClearall.Click
        ...
        If (listViewChInfo_DO.SelectedIndices.Count = 0) Then
            ...
            Return
        End If
        If m_adamModbusSocket.Alarm.ClearAlarmFlags(Me.m_iSlot_ID, 15) Then
            RefreshData()
        Else
    ...
    Private Sub btnApplySelRange_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplySelRange.Click
        ...
        If bRet Then
            ...
            End If
            Dim iChannelTotal As Integer = Me.m_aConf.HwIoTotal(m_CNTidx)
            If m_adamModbusSocket.Counter.SetModes(Me.m_iSlot_ID, iChannelTotal, usRanges) Then
            ...
            Else
                MessageBox.Show(("Set modes failed! ApiErr:" _
                                + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
            End If
        End If
        Timer1.Enabled = True
    End Sub
    ...
    Private Sub btnApplyStartup_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplyStartup.Click
        ...
        If bRet Then
            ...
            End If
            Dim iChannelTotal As Integer = Me.m_aConf.HwIoTotal(m_CNTidx)
            If m_adamModbusSocket.Counter.SetStartupValues(Me.m_iSlot_ID, uiVals) Then
                RefreshCntStartupValues()
            Else
    ...
    Private Sub settingCNT_MaskSetting(ByVal bEnable As Boolean, ByVal i_bApplyAll As Boolean)
        ...
        If bRet Then
            ...
            Do While (iIdx < bChMask.Length)
                ...
                iIdx = (iIdx + 1)
            Loop
            If m_adamModbusSocket.Counter.SetChannelEnabled(Me.m_iSlot_ID, uiMask) Then
                RefreshRanges()
            Else
    ...
    Private Sub btnResetCnt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnResetCnt.Click
        ...
        If bRet Then
            ...
            Do While (iIdx < bChMask.Length)
                ...
                iIdx = (iIdx + 1)
            Loop
            If Not m_adamModbusSocket.Counter.SetToStartup(Me.m_iSlot_ID, uiMask) Then
    ...
    Private Sub btnClearOF_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClearOF.Click
        ...
        If bRet Then
            ...
            Do While (iIdx < bChMask.Length)
                ...
                iIdx = (iIdx + 1)
            Loop
            If Not m_adamModbusSocket.Counter.ClearOverflows(Me.m_iSlot_ID, uiMask) Then
                MessageBox.Show(("Clear counter overflow flag failed! ApiErr:" + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
            End If
        End If
        Timer1.Enabled = True
    ...
    Private Sub btnApplyCountType_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplyCountType.Click
        ...
        If bRet Then
            If bApplyAll Then
                Dim i As Integer = 0
                Do While (i < listViewChInfo_CNT.Items.Count)
                    If Not m_adamModbusSocket.Counter.SetCntTypeConfiguration(m_iSlot_ID, i, chbxRepeat.Checked, chbxReloadToStartup.Checked) Then
            ...
            Else
                Dim i As Integer = 0
                Do While (i < listViewChInfo_CNT.SelectedIndices.Count)
                    If Not m_adamModbusSocket.Counter.SetCntTypeConfiguration(Me.m_iSlot_ID, listViewChInfo_CNT.SelectedIndices(i), chbxRepeat.Checked, chbxReloadToStartup.Checked) Then
                        MessageBox.Show(("Set count type" _
                    ...
                    If Not ((m_usRanges(listViewChInfo_CNT.SelectedIndices(i)) = CType(ApaxUnknown_InputRange.Up, System.UInt16)) _
                        ...
                        If Not m_adamModbusSocket.Counter.SetCntTypeConfiguration(m_iSlot_ID, neighborIdx, chbxRepeat.Checked, chbxReloadToStartup.Checked) Then
                            MessageBox.Show(("Set count type" _
                                            + (listViewChInfo_CNT.SelectedIndices(i).ToString + (" configuration failed! ApiErr:" _
                                            + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")))), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
                            Timer1.Enabled = True
    ...
    Private Sub btnClrTriger_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClrTriger.Click
        Try
            ...
            Else
                ...
                Loop
            End If
            m_adamModbusSocket.Configuration.TOOL_SetCmdWriteReadRet(Me.m_iSlot_ID, &H78, uiInput)
        Catch ex As Exception
    ...
    Private Sub btnApplyGateSetting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplyGateSetting.Click
        ...
        If bRet Then
            If bApplyAll Then
                Dim i As Integer = 0
                Do While (i < listViewChInfo_CNT.Items.Count)
                    If Not m_adamModbusSocket.Counter.SetLocalGateConfiguration(Me.m_iSlot_ID, i, chbxGateEnable.Checked, CType(cbxTriggerMode.SelectedIndex, Byte), CType(cbxGateType.SelectedIndex, Byte), cbxLocalGateMapCh.SelectedIndex) Then
            ...
            Else
                Dim i As Integer = 0
                Do While (i < listViewChInfo_CNT.SelectedIndices.Count)
                    If Not m_adamModbusSocket.Counter.SetLocalGateConfiguration(Me.m_iSlot_ID, listViewChInfo_CNT.SelectedIndices(i), chbxGateEnable.Checked, CType(cbxTriggerMode.SelectedIndex, Byte), CType(cbxGateType.SelectedIndex, Byte), cbxLocalGateMapCh.SelectedIndex) Then
                        MessageBox.Show(("Set gate" _
                                        + (listViewChInfo_CNT.SelectedIndices(i).ToString + (" configuration failed! ApiErr:" _
                                        + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")))), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
                        Timer1.Enabled = True
                    ...
                    If Not ((m_usRanges(listViewChInfo_CNT.SelectedIndices(i)) = CType(ApaxUnknown_InputRange.Up, System.UInt16)) _
                        ...
                        If Not m_adamModbusSocket.Counter.SetLocalGateConfiguration(Me.m_iSlot_ID, neighborIdx, chbxGateEnable.Checked, CType(cbxTriggerMode.SelectedIndex, Byte), CType(cbxGateType.SelectedIndex, Byte), cbxLocalGateMapCh.SelectedIndex) Then
                            MessageBox.Show(("Set gate" _
                                            + (listViewChInfo_CNT.SelectedIndices(i).ToString + (" configuration failed! ApiErr:" _
                                            + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")))), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
                            Timer1.Enabled = True
    ...
    Private Sub listViewChInfo_DoubleClick(ByVal sender As Object, ByVal e As EventArgs)  ' Double click channel to change value
        If Not CheckControllable() Then
            Return
        End If
    ...
    Private Sub btnApplyFilter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplyFilter.Click
        ...
        Try
            ...
            If (iFilter > 400000) Then
                ...
                Return
            Else
                bRet = m_adamModbusSocket.Counter.SetDigitalFilter(Me.m_iSlot_ID, iFilter)
                If bRet Then
                    If (iFilter = 0) Then
                ...
                Else
                    MessageBox.Show(("Set digital filter failed! ApiErr:" _
                                    + (m_adamModbusSocket.Counter.ApiLastError.ToString + "")), "Error", MessageBoxButtons.OK, MessageBoxIcon.Hand, MessageBoxDefaultButton.Button1)
                End If
                Return
            End If
        Catch ex As System.Exception
    ...
    Private Sub btnFreqAcqTime_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFreqAcqTime.Click
        ...
        Try
            ...
            If ((iFreqAcqTime < 1) _
                ...
                Return
            End If
            Dim bRet As Boolean = m_adamModbusSocket.Counter.SetFreqAcqTime(Me.m_iSlot_ID, iFreqAcqTime)
            If bRet Then
                RefreshCntSetting()
            Else
    ...
    Private Sub cbxDOBehavior_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub cbxLocalAlarmMapCh_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub listViewChInfo_DO_DoubleClick(ByVal sender As Object, ByVal e As EventArgs) Handles listViewChInfo_DO.ItemActivate
        If Not CheckControllable() Then
            Return
        End If
        Dim strVal As String = listViewChInfo_DO.Items(listViewChInfo_DO.SelectedIndices(0)).SubItems(3).Text
    ...
    Private Sub btnApplySetting_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnApplySetting.Click
        ...
        If bApplyAll Then
            Dim i As Integer = 0
            Do While (i < listViewChInfo_DO.Items.Count)
                If Not m_adamModbusSocket.Alarm.SetLocalAlarmConfiguration(Me.m_iSlot_ID, i, bIsAlarm, bAutoRL, CType(iAlarmType, Byte), iMappingCh, uiLimitVal, CType(iDOBehavior, Byte), uiPulseWidth) Then
                    MessageBox.Show(("Set alarm" _
                                    + (i.ToString + (" configuration failed! ApiErr:" _
                                    + (m_adamModbusSocket.Alarm.ApiLastError.ToString + "")))), "Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk, MessageBoxDefaultButton.Button1)
                    Timer1.Enabled = True
                    Return
                End If
                i = (i + 1)
        ...
        Else
            Dim i As Integer = 0
            Do While (i < listViewChInfo_DO.SelectedIndices.Count)
                If Not m_adamModbusSocket.Alarm.SetLocalAlarmConfiguration(Me.m_iSlot_ID, listViewChInfo_DO.SelectedIndices(i), bIsAlarm, bAutoRL, CType(iAlarmType, Byte), iMappingCh, uiLimitVal, CType(iDOBehavior, Byte), uiPulseWidth) Then
                    MessageBox.Show(("Set alarm" _
    ...
    Private Sub Form_APAX_5080_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        FreeResource()
    End Sub
End Class

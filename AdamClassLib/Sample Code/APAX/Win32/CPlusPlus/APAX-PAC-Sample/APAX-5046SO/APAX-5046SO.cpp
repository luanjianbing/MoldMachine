// APAX-5046SO.cpp : Defines the entry point for the console application
...
const int g_ChannelNum = 24; //the channel number of APAX_5046SO is 24
...
void EX_GetValue(LONG i_lDriverHandle, WORD i_wSlotID)
{
	...
	{
		//check all channels (there are 24 channels in Apax-5046)
...
void EX_SetMultiValue(LONG i_lDriverHandle, WORD i_wSlotID) 
{
	...
	DWORD dLowValue = 0x000F0F0F;	//the lower DO value from channel 0 to 31
	...
	if (ERR_SUCCESS == lSetMultiValueResult )
	{
		...
		printf("Fail to set DO multiple channels, error code = %d\n", lSetMultiValueResult);
}
...
void EX_GetMultiValue(LONG i_lDriverHandle,WORD i_wSlotID)
{
	...
	{
		//show all channels (there are 24 channels in Apax-5046)
		for (iCnt = 0; iCnt < g_ChannelNum ; iCnt++)
		{
		...
		printf("Fail to get DO multiple channels, error code = %d\n", lGetMultiValueResult);
...
int main(int argc, char* argv[])
{
	...
	if (bAdvStatus = EX_OpenLib(&lDriverHandle))
	{
		...
		//the moduleID for Apax-5046SO is 0x5046534F
		//dHiWord is 5046, dLoWord is 534F
